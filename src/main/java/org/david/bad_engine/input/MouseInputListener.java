...
/**
 * Listens to user's mouse inputs.
 *
 * @author Mr. GodDavid
 * @since 3/31/2026
 */
...
public final class MouseInputListener implements MouseListener, MouseMotionListener {
    ...
    private static final boolean[] mouseButtonsConsumed = new boolean[MouseInputListener.MouseButton.values().length];
    private volatile static boolean isMouseMoved;
    private final Vector2d mouseCursorPosition;
    ...
    public void mouseReleased(MouseEvent e) {
        int index = e.getButton() - 1;
        if (index >= 0 && index < mouseButtonsConsumed.length) {
            mouseButtonsPressed[index] = false;
            mouseButtonsConsumed[index] = false;
        }
    }
    ...
    public void mouseDragged(MouseEvent e) {
        mouseCursorPosition.setValues(e.getX(), e.getY());
        isMouseMoved = true;
    }
    ...
    /**
     * Accessor for {@code mouseCursorPosition}
     */
    ...
    /**
     * Fires a mouse event {@code ONE TIME} when a mouse button is been pressed by user.
     *
    ...
    public static boolean isButtonDown(MouseButton button) {
        int index = button.getButton() - 1;
        if (index < 0 || index >= mouseButtonsPressed.length) return false;
        if (mouseButtonsPressed[index] && !mouseButtonsConsumed[index]) {
            mouseButtonsConsumed[index] = true;
            return true;
        }
        return false;
    }
    ...
    /**
     * Fires a mouse event {@code MULTIPLE TIMES} when a mouse button is being pressed by user.
     *
     * @param button that indicates which mouse button the user presses.
     * @return true for the correspond mouse button being pressed by user.
     */
    public static boolean isButtonPressed(MouseButton button) {
        int index = button.getButton() - 1;
        if (index < 0 || index >= mouseButtonsPressed.length) {
            return false;
        }
        return mouseButtonsPressed[index];
    }

    /**
     * Checks if the user moves the mouse.
     *
